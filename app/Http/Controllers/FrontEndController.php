...
class FrontEndController extends Controller
{
     ...
     */
    private function filterRestaurantsOnQuery(Request $request, $restaurantIDS)
    {
        //1. Find all items
        $items = Items::where(['available' => 1])->where(function ($q) use ($request) {
        ...
        $restorantsQ = Restorant::where(['active' => 1])->where(function ($q) use ($request) {
            $stripedQuery = '%' . strip_tags($request->input('q')) . "%";
            $q->where('name', 'like', $stripedQuery)->orWhere('description', 'like', $stripedQuery);
        });
    ...
    public function index(Request $request)
    {
        ...
        if (env('MULTI_CITY', false) && !($hasLocation || $hasQuery)) {

            return $this->multyCityMode($request);
        }

        //ELSE - Query and Location mode //Default
        return $this->showStores($request, null);
    }
     ...
     * 4. Multy city mode
     */
    public function multyCityMode(Request $request)
    {
        //Default headers
     ...
     */
    public function showStores(Request $request, $city_alias)
    {
        //Variants
        /**
        ...
        if ($hasCity) {
            ...
            if ($hasQuery) {
                //With Query
                $restorants = $this->filterRestaurantsOnQuery($request, $theRestorants->pluck('id')->toArray());
            } else {
                //No query
            ...
            $restorants = $this->filterRestaurantsOnQuery($request, Restorant::where(['active' => 1])->pluck('id')->toArray());
