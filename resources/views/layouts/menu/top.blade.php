<nav id="navbar-main" class="navbar navbar-light navbar-expand-lg fixed-top">
    ...
    <div class="container-fluid">
        ...
        @if (request()->get('location'))
            <span style="z-index: 10" class="">{{ __('DELIVERING TO') }} :
                <b>{{ request()->get('location') }}</b></span> <a data-toggle="modal" href="#locationset"><span
                    class="ml-sm-2 search description">({{ __('change') }})</span></a>
        @endif
        ...
        <div class="navbar-collapse collapse" id="navbar_global">
            ...
            <ul class="navbar-nav align-items-lg-center ml-lg-auto">
                @if (!env('SINGLE_MODE', false))
                    <li class="nav-item">
                        <a data-mode="popup" target="_blank" class="button nav-link nav-link-icon btn"
                            href="{{ route('newrestaurant.register') }}">{{ __('Register as Restaurant') }}</a>
                    </li>
                @endif
